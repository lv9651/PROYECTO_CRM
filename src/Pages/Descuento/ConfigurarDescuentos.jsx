...
const getApiProductId = useCallback((producto) => {
  // Fuerza el uso del ID real (50002116) para API
  return producto.id ||producto.idUnico; // Esto siempre devolverá "50002116" en tu estructura
}, []);
  ...
  // Obtener descuentos para un producto específico
...
const handlePorcentajeProvChange = useCallback((e) => {
  ...
  const porcentajes = porcentajesPorProducto[prod.idUnico] || { 
  qf: porcentajesGlobales.qf, 
  prov: porcentajesGlobales.prov 
};
      ...
      return {
        ...
        PVV: primerDescuento.precio || '',
        precio: primerDescuento.precio || null,
        total: primerDescuento.total || null,
  ...
  }, [productosLaboratorio, productosSeleccionados, descuentosAplicados, getDescuentosProducto, getProductId]);
...
const handleGuardarLab = useCallback(async () => {
  ...
  try {
    ...
    // 4. Actualizar estado solo con productos válidos
    if (productosValidos.length > 0) {
      setProductosLaboratorio(prev => [...prev, ...productosValidos]);
      handleCloseLabModal();
    } else {
...
}, [selectedLabs, fetchProductosPorLaboratorio, handleCloseLabModal, listas]);
  ...
  // Efecto para cargar productos al abrir el modal
  useEffect(() => {
                ...
                <TableBody>
                  {productosConLista.map((prod) => (
                    ...
                    >
                      ...
                      <TableCell>{prod.PVV || '0'}</TableCell>
